package MediaMogul::Plugin::Message;
...
sub message {
    ...
    $c->stash->{$stash_key} ||= $c->model('DataManager')->messages;
...
sub dispatch {
    ...
    my $rflash_key = $c->config->{'Plugin::Message'}->{results_flash_key} || '_results';
    my $rstash_key = $c->config->{'Plugin::Message'}->{results_stash_key} || 'results';
